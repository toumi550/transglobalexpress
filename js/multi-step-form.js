...
class MultiStepForm {
    ...
    showValidationErrors() {
        ...
        if (this.currentStep === 1 && !this.selectedService) {
            this.showMessage(getTranslation('messages.form.select_service'), 'error');
        }
    }
    ...
    async submitForm(e) {
        ...
        // Show loading state
        this.submitBtn.innerHTML = `<i class="fas fa-spinner fa-spin"></i> ${getTranslation('messages.form.sending')}`;
        ...
        try {
            ...
            this.showMessage(getTranslation('messages.form.error'), 'error');
        } finally {
            this.submitBtn.innerHTML = `<i class="fas fa-paper-plane"></i> ${getTranslation('messages.buttons.send_request')}`;
    ...
    showSuccessModal(quoteNumber) {
        ...
        modal.innerHTML = `
            <div class="success-modal-content">
                ...
                <h3>${getTranslation('messages.quote.success')}</h3>
                <p>${getTranslation('messages.quote.number')}: <strong>${quoteNumber}</strong></p>
                <p>${getTranslation('messages.quote.contact_time')}</p>
                <div class="success-actions">
                    <button class="btn btn-primary" onclick="this.closest('.success-modal').remove()">
                        ${getTranslation('messages.buttons.perfect')}
                    </button>
                    <button class="btn btn-secondary" onclick="window.print()">
                        ${getTranslation('messages.buttons.print')}
                    </button>
                </div>
            </div>
        `;
