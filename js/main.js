...
class QuoteFormHandler {
    ...
    handleSubmit(e) {
        ...
        submitBtn.innerHTML = `<span>${getTranslation('messages.form.sending')}</span><i class="fas fa-spinner fa-spin"></i>`;
...
function fallbackCopyTextToClipboard(text) {
    ...
    try {
        ...
        console.error('Erreur lors de la copie:', err);
        showNotification(getTranslation('messages.form.copy_error'), 'error');
    }
...
document.addEventListener('DOMContentLoaded', function () {
    // Initialiser EmailJS
emailjs.init("nRUhL0dzQz2Ud13RO"); // Clé publique EmailJS configurée
    ...
    if (quickContactForm) {
        quickContactForm.addEventListener('submit', function (e) {
            ...
            if (!data.name || !data.phone || !data.subject || !data.message) {
                showNotification(getTranslation('messages.form.required_fields'), 'error');
                return;
            }
            ...
            submitButton.innerHTML = `<i class="fas fa-spinner fa-spin"></i> ${getTranslation('messages.form.sending')}`;
            ...
            const templateParams = {
                ...
                to_email: 'contact@transglobalexpress-dz.com'
            };
        ...
        emailjs.send('service_3nxazte', 'template_5tep3xq', templateParams)
                .then(function(response) {
                    ...
                    setTimeout(() => {
                        ...
                        const whatsappUrl = `https://wa.me/213550968092?text=${encodeURIComponent(whatsappMessage)}`;

                        if (confirm(getTranslation('messages.form.whatsapp_confirm'))) {
    ...
    statusBadges.forEach((badge, index) => {
        if (index < 5) { // Lundi à Vendredi
            if (currentDay >= 1 && currentDay <= 5 && currentDay === index + 1) {
                ...
                badge.textContent = isOpen ? getTranslation('messages.status.open') : getTranslation('messages.status.closed');
            }
